...
#
################################################################################
# This file is almost completely as it's original form, as released by the
# nice people at [https://www.reverse-engineering.net].  I've merely
# modified it to add a little more color.  For this reason, the version
# numbers I will always be based on the "upstream" version number.
#
# REVISION: 7.1-0.3
#
# CHANGELOG:
#
#   Version 7.1-0.3
#     Major bug fix in memory dump.
#     EIP address is now also coloured, and the code data is also dumped raw.
#
#   Version 7.1-0.2
#     Added remaining EFLAGS to context view.
#
#   Version 7.1-0.1
#     Highlight ESP, EIP, EBP registers, in alignment with the stack and
#       code dumps.
#     Realign register display arrangements.
#     Colourize EFLAGS.
#
# MANGLER: achaemenes @ #autonomy @ freenode
#
################################################################################
# ORIGINAL HEADER
#
...
define hex_quad
    if $argc != 1
        ...
        while ($_i < 8)
            if ($_i % 4 == 0)
                set $_b4 = (unsigned int)($arg0 + (char)$_i)
                if ($_b4 == $esp)
                    echo \033[1;33m
                else
                    if ($_b4 == $ebp)
            ...
            printf "%02X", *(unsigned char*)($arg0 + (char)$_i)
...
define ddump
    if $argc != 1
        ...
        set $_count = 0
        while ($_count < $arg0)
            set $_dwi = ($_count * 0x10)
            hexdump $data_addr+$_dwi
            set $_count++
        end
    end
end
...
# Display instructions in AT&T(att)/Intel(intel) format
